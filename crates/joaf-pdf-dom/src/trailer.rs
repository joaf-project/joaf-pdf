use joaf_pdf_core::{ObjectId, PdfDictionary, PdfError, PdfName, PdfObject};
...
impl<'a> TryFrom<&PdfDictionary<'a>> for Trailer {
    ...
    fn try_from(value: &PdfDictionary) -> Result<Self, Self::Error> {
        let size = match value.get_required(&PdfName::SIZE) {
            Ok(PdfObject::Integer(size)) => *size as u32,
            _ => return Err(PdfError::from("Size is not an integer.")),
        };

        let root = match value.get_required(&PdfName::ROOT) {
